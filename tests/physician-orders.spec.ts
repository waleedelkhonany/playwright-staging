...
test.describe('E2E: Create Dialysis Order', () => {
  ...
  // The modal is Livewire-driven: filling + read-back verification + targeted
  // repair routinely takes ~55–60s, so raise the per-test timeout above the
  // global 60s default to avoid timing out on slow staging responses.
  ...
  for (const scenarioFile of DIALYSIS_SCENARIOS) {
    test(`should create a Dialysis Order end-to-end (${scenarioFile})`, async ({ page, patientsPage }) => {
      // =========================================================================
      ...
      const order = getDialysisOrderData(scenarioFile);
      ...
      }).format(new Date()); // YYYY-MM-DD (Asia/Riyadh)
      ...
      // =========================================================================
      // 5. Fill the form (fill → read-back verify → auto-repair any field the
      //    Livewire re-render reset, so Save never trips a validation popup)
      // =========================================================================
