/**
 * Dump the DOM of the "VASCULAR ACCESS ASSESSMENT TOOL" form on staging.
 *
 * The form is reached via the "VASCULAR ACCESS ASSESSMENT TOOL" tab on the
 * visit edit page (href "load/visit-form/{id}/vascular-access-assessment"),
 * so this probe navigates straight to /load/visit-form/{id}/vascular-access-assessment
 * and dumps:
 *   - page headers
 ...
 *   - Save buttons
 *
 * Run: npx tsx scripts/inspect-vascular-access.ts [visitId]
 */
...
async function main() {
  ...
  try {
    ...
    await page.screenshot({ path: 'test-results/artifacts/vascular-access-form.png', fullPage: true });
    console.log('\nScreenshot saved to test-results/artifacts/vascular-access-form.png');
  } catch (err) {
    console.error('Error:', err);
    await page.screenshot({ path: 'test-results/artifacts/inspect-vascular-access-error.png' }).catch(() => {});
  } finally {
    await browser.close();
  }
}
