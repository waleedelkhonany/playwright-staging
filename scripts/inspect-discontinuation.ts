/**
 * Inspect the Discontinuation of Hemodialysis Session form workflow on staging.
 *
 * 1. Login
 ...
 * 5. Navigate directly to the discontinuation form URL
 *    (/load/visit-form/{id}/discontinuation-of-hemodialysis-session) and dump
 *    every input/select/textarea (name, wire:model, id, type, placeholder,
 *    radio groups + label texts, sections)
 * 6. Dump the Save button(s)
 *
 * Run: npx tsx scripts/inspect-discontinuation.ts [visitId]
 */
...
async function main() {
  ...
  try {
    ...
    // 5. Try candidate URLs for the discontinuation form and dump whichever exists
    const candidates = [
      `discontinuation-of-hemodialysis-session`,
      `discontinuation-hemodialysis-session`,
      `discontinuation`,
      `hemodialysis-discontinuation`,
      `discontinuation-of-hemodialysis`,
      `discontinuation-of-dialysis-session`,
    ];
    let loaded = false;
    for (const slug of candidates) {
      ...
      if (fieldCount > 5) {
        loaded = true;
        await dumpForm(page, `DISCONTINUATION FORM at ${url}`);
        break;
      }
    }
    ...
    if (!loaded) {
      ...
      const body = await page.evaluate(() => (document.body?.textContent || '').replace(/\s+/g, ' ').slice(0, 500));
      console.log(body);
    }

    await page.screenshot({ path: 'test-results/artifacts/discontinuation-page.png', fullPage: true });
    console.log('\nScreenshot saved to test-results/artifacts/discontinuation-page.png');
  } catch (err) {
    console.error('Error:', err);
    await page.screenshot({ path: 'test-results/artifacts/inspect-discontinuation-error.png' }).catch(() => {});
  } finally {
    await browser.close();
  }
}
