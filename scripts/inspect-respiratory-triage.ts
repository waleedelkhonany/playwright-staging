/**
 * Dump the DOM of the "Respiratory Triage" form on staging.
 *
 * The form is reached via the "Respiratory Triage" tab on the visit edit page
 * (href "load/visit-form/{id}/respiratory-triage"), so this probe navigates
 * straight to /load/visit-form/{id}/respiratory-triage and dumps:
 *   - page headers
 ...
 *   - Save buttons
 *
 * Run: npx tsx scripts/inspect-respiratory-triage.ts [visitId]
 */
...
async function main() {
  ...
  try {
    ...
    await page.screenshot({ path: 'test-results/artifacts/respiratory-triage-form.png', fullPage: true });
    console.log('\nScreenshot saved to test-results/artifacts/respiratory-triage-form.png');
  } catch (err) {
    console.error('Error:', err);
    await page.screenshot({ path: 'test-results/artifacts/inspect-respiratory-triage-error.png' }).catch(() => {});
  } finally {
    await browser.close();
  }
}
